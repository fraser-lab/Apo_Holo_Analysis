...
def main():
    ...
    try:
        B_factor = pd.read_csv(args.holo + qFit + "__B_factors.csv")
    except IOError:
        pass
    try:
        sasa = pd.read_csv(args.holo + qFit + "_sasa.csv", index_col=0)
    except IOError:
        pass

    try:
        rmsf = pd.read_csv(args.holo + qFit + "_qfit_RMSF.csv")
    except IOError:
        pass

    try:
        rotamer = pd.read_csv(args.holo + qFit + "_rotamer_output.txt", sep = ':')
    ...
    try:
        order = pd.read_csv(args.holo + qFit + '_methyl.out', sep=',')
    except IOError:
        pass
    ...
    rmsf_subset.to_csv(args.holo + '_' + args.dist + '_rmsf_subset.csv', index=False)
